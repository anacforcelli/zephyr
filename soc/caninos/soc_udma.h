...
/*** UDMA EVENT IDS ***/
// id : number relative to number of same peripheral type (NOT UDMA ID)
#define ARCHI_UDMA_UART_RX_EVT(id)    ((UDMA_UART_ID + id) * 4)
#define ARCHI_UDMA_UART_TX_EVT(id)    (ARCHI_UDMA_UART_RX_EVT(id) + 1)

#define ARCHI_UDMA_SPIM_RX_EVT(id)    ((UDMA_SPIM_ID + id) * 4)
 ...
 *
	...
	*/
static inline void plp_udma_enqueue(unsigned channelBase, uint32_t l2Addr, unsigned int size, unsigned int cfg)
{
